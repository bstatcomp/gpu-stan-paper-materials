...
models   <- list(m1 = create_model("./stan/lr_basic.stan", 
                                   cmdstan_dir = path_to_cmdstan),
                 m2 = create_model("./stan/lr_glm.stan", 
                                   cmdstan_dir = path_to_cmdstan))

# List datasets
datagens <- list(d0 = dg_simple_linear(variants = expand.grid(seed = 0,           
                                                              n = 4^c(3:7),       # no. of observations
...
g1 <- ggplot(res$all_results, aes(x = n, y = time_in_ms, colour = paste(cmdstan_dir, stan_file, sep = ";"))) + 
  geom_point() + geom_smooth(span = 5) + facet_wrap(.~k)
plot(g1)
